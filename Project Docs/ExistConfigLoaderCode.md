# Source of Truth - Реализованные классы в `config_loader.py`

> Модуль `config_loader.py` отвечает за чтение манифеста и конфигурационных файлов, их валидацию, нормализацию и проекцию в строго типизированные DTO.
> Пункт [### Техническая реализация] прямо отражает настоящий текущий код класса.

---

# Группа классов для работы с ошибками:

На текущем этапе в `config_loader.py` реализована группа классов для обработки ошибок:

* `ConfigError` — универсальное исключение для бизнес-логики.
* `StructuredError` — DTO для представления ошибки в унифицированном виде.
* `_ErrorFormatter` — слой нормализации исключений в `StructuredError`.

> Эти классы обеспечивают основу для детерминированной диагностики и трассируемости конфигурационных ошибок. В дальнейшем на них будет опираться вся система загрузки и валидации конфигов

---

## Внутренний класс `ConfigError`

### Роль в модуле

* Универсальное бизнес-исключение для всех этапов работы `ConfigLoader`.
* Используется в местах проверки инвариантов, семантики, выбора профилей и других бизнес-операций.
...
* Поле `code` всегда задаётся из спецификации (например, `SANITY_MISSING_KEY`).
* Сообщение (`message`) должно быть коротким и информативным.
* `json_pointer` и `profile_id` указываются при наличии контекста.

### Допущения

* Реализация проверки корректности `code`  out of scope - не ожидается.
* Все ошибки, не являющиеся бизнесовыми, могут использовать общий fallback.

### Бизнес-логика

* Ошибка поднимается на уровне проверки, например:

```python
if "meta" not in config:
    raise ConfigError("SANITY_MISSING_KEY", "Missing required 'meta'", json_pointer="/meta")
```

---

## Внутренний класс `StructuredError`

### Роль в модуле

* Унифицированная модель ошибки, пригодная для сериализации и передачи наружу.
* Используется как стандартный результат форматирования.
...
* Формат соответствует контракту: `{code, message, config_id, type, profile_id?, json_pointer?}`.
* Может быть сериализован напрямую в JSON.

### Допущения

* Не выполняет валидацию входных данных — это зона ответственности `_ErrorFormatter` и источников ошибок.

### Бизнес-логика

* Не содержит бизнес-правил, чистый DTO.

---

## Внутренний класс `_ErrorFormatter`

### Роль в модуле

* Преобразует любое исключение (`ConfigError`, `ValidationError`, системное `Exception`) в `StructuredError`.
* Обеспечивает единообразный формат ошибок независимо от источника.
...
* Работает детерминированно: один и тот же вход → один и тот же `StructuredError`.
* Поддерживает как бизнес-ошибки (`ConfigError`), так и валидационные (`jsonschema`, `pydantic`).
* При неизвестных исключениях использует fallback `IO_INVALID_DOC`.

### Допущения

* Поддержка `jsonschema` и `pydantic` завязана на наличие этих пакетов.
* Для `pydantic` и `jsonschema` учитывается только первая ошибка (остальные могут теряться).
* Нет встроенной маскировки секретов - out of scope.

### Бизнес-логика

* Не содержит бизнес-правил — только упаковка.
* Весь смысл ошибки (код, контекст) задаётся в источнике (`ConfigError` или валидаторе).

---

# Класс работы с Manifest.

> На текущем этапе в `config_loader.py` реализована подсистема чтения и валидации манифеста конфигураций.

## Внутренний класс `_ManifestReader`

### Роль в модуле

Класс отвечает за детерминированное чтение JSON-манифеста, его валидацию по схеме `manifest/v1/manifest.schema.json`, проекцию данных в строго типизированные DTO (`ConfigManifestV1`, `ManifestMeta`, `ConfigEntry`) и выполнение минимальных бизнес-инвариантов. Он является входной точкой для получения корректного представления списка конфигов, с которыми затем работает загрузчик.
...
class _ManifestReader:
    ...
    Ответственность класса:
      ...
      - Check: минимальные бизнес-инварианты (обязательные типы, уникальность config_id).
...
* Вызов `read()` возвращает валидный объект DTO `ConfigManifestV1`.
* При любой ошибке загрузка немедленно останавливается (fail-fast).
* Ошибки всегда имеют структурированный формат (`ConfigError`), пригодный для преобразования в `StructuredError` через `_ErrorFormatter`.
* Порядок `configs` в DTO сохраняется идентичным исходному JSON.

### Допущения

* Поддерживается только версия схемы v1 (`manifest/v1/manifest.schema.json`).
* Путь к схеме передаётся через DI (`manifest_schema_path`).
* Поле `tags` всегда присутствует (схема гарантирует `default=[]`), поэтому доступ идёт напрямую.
* Семантические проверки (`config_id` ↔ `type`, корректность версий и др.) выполняются на следующих этапах, а не в `_ManifestReader`.

### Бизнес-логика

* Гарантирует, что в манифесте есть хотя бы один конфиг каждого обязательного типа: `pipeline`, `network`, `site`, `url`, `auth`, `logging`, `export`.
* Гарантирует уникальность `config_id` во всём списке `configs`.
* Любая ошибка приводит к немедленному прерыванию загрузки с чётким сообщением и координатой ошибки в документе.
* Проецирует данные в DTO из `manifest_dto.py` (через классы `ConfigType`, `ConfigEntry`, `ManifestMeta`, `ConfigManifestV1`).
* При ошибках бросает `ConfigError` с кодами (`IO_*`, `SCHEMA_*`, `SANITY_*`) и, где возможно, указывает `json_pointer`.

---

# Класс чтения конфигов

> На текущем этапе в `config_loader.py` реализовано чтение и базовая структурная проверка отдельных конфигов.

## Внутренний класс `_ConfigReader`

### Роль в модуле

Класс отвечает за чтение файлов конфигурации, пути к которым заданы в манифесте, и приведение их к валидному Python-объекту (`dict`). Этот шаг фиксирует корректность доступа к файлу, его существование, базовую структурную валидность и синтаксис JSON.
...
* На вход получает DTO `ConfigEntry` из манифеста.
* Возвращает `dict[str, Any]` — объект верхнего уровня JSON-конфига.
* В случае любой ошибки — fail-fast выброс `ConfigError` с кодом, текстом и (при необходимости) указателем строки/колонки.

### Допущения

* Все конфиги формата v1 обязаны быть JSON-файлами в кодировке UTF-8.
* Поддержка только обычных текстовых файлов (`.json`), без сжатых (`.json.gz`) или бинарных форматов.
* `{}` (пустой объект) считается допустимым и передаётся дальше, фильтрация его смысла отложена на уровень валидации схемы.
* `null`, пустая строка и пустой массив считаются некорректными документами.

### Бизнес-логика

* Fail-fast модель: первая ошибка в конфиге останавливает обработку.
* Детерминированность: одинаковый путь при одинаковом содержимом → одинаковый результат.
* Трассируемость: все ошибки содержат путь к файлу, коды ошибок стандартизированы.
* Безопасность: загрузка строго ограничена JSON, без выполнения произвольного кода.

---

# SanityCheck

> На текущем этапе в `config_loader.py` реализован класс `_ConfigSanityCheck`, отвечающий за минимальные структурные проверки до выполнения JSON Schema валидации.

## Внутренний класс `_ConfigSanityCheck`

### Роль в модуле

Класс выполняет **раннюю проверку корректности структуры конфигурации** (этап 2.2 в pipeline). Его цель — выявить критические несоответствия и остановить обработку на максимально раннем этапе (fail-fast), чтобы не тратить ресурсы на дальнейшую обработку некорректных данных.
...
* Класс должен использоваться **непосредственно после чтения конфига** (`_ConfigReader`) и перед JSON Schema валидацией.
* Ошибки должны быть детерминированы и трассируемы: в исключение передаётся `json_pointer`, указывающий на проблемное поле.
* Интерфейс класса минималистичен: метод `run()` бросает исключение при ошибке или возвращает `None` при успехе.

### Допущения

* Формат секции `meta` гарантированно соответствует базовой схеме (`config_id`, `version` и др.).
* Проверка уникальности `config_id` среди всех конфигов выполняется на этапе `_ConfigReader` и здесь не дублируется.
* Проверка принадлежности `pipeline_step` к `ConfigType` выполняется на этапе `_ConfigReader` и здесь не дублируется.
* Поддержка версий ограничена простыми масками: `M.m.p`, `M.x`, `M.m.x`. Более сложные правила миграции не рассматриваются.

### Бизнес-логика

* Fail-fast принцип: любая ошибка структуры останавливает загрузку всех конфигов.
* Экономия ресурсов: выполняются только проверки за O(1), без тяжёлых операций.
* Поддержка эволюции схем: возможность задавать маски версий обеспечивает совместимость с будущими обновлениями.
* Прозрачность: ошибки формируются в унифицированном формате, чтобы `_ErrorFormatter` мог собрать единый отчёт для внешнего уровня.

---

# Проверка конфигурационных файлов по JSON Schema.

> На текущем этапе в `config_loader.py` реализована поддержка универсальной строгой проверки конфигурационных файлов по JSON Schema.

## Внутренний класс `_ConfigUniversalJsonSchemaValidation`

### Роль в модуле

Класс выполняет под-этап пайплайна загрузки конфигов. Его основная задача — подтвердить, что JSON-документ строго соответствует своей спецификации (`*.schema.json`) в формате Draft 2020-12. Это исключает структурные ошибки до перехода к нормализации и проекции в DTO.
...
* Схемы находятся в фиксированной структуре каталогов (`configs/v1`).
* Валидация всегда проводится по Draft 2020-12.
* При ошибке должен быть понятный отчёт: код, сообщение, `json_pointer` (если применимо).
* Класс не изменяет входные данные — только проверяет.

### Допущения

* Все схемы доступны локально, в соседних `.schema.json` файлах.
* Семантические проверки (например, согласованность версий или бизнес-ограничений) выполняются позже в пайплайне.
* `$id` в схемах уникальны и корректны.
* Допускается наличие повреждённых или лишних `.schema.json` файлов — они пропускаются.

### Бизнес-логика

* Обеспечить **fail-fast**: первая ошибка останавливает процесс загрузки конфигов.
* Обеспечить трассируемость: фиксируется тип конфига, путь к схеме и место ошибки.
* Гарантировать детерминированность: один и тот же документ и схема всегда дают одинаковый результат.
* Повысить безопасность пайплайна: только валидные по структуре документы допускаются к дальнейшей обработке.

---

# Активация профилей конфигураций

> На текущем этапе в `config_loader.py` реализована логика выбора и нормализации активных профилей в конфигурации.

## Внутренний класс `_ConfigActiveProfileSelector`

### Роль в модуле

Класс отвечает за корректный выбор и нормализацию активных профилей в конфиге. Он используется после чтения и базовой валидации конфига и гарантирует, что:

* всегда будет выбран предсказуемый набор профилей;
* при несоответствиях процесс немедленно прервётся fail-fast ошибкой.
...
* Вызов всегда возвращает список профилей в детерминированном порядке.
* При ошибках обработка немедленно прерывается.
* Результат не изменяет входные данные `data`.

### Допущения

* JSON Schema гарантирует, что `profiles` и `active_profiles` существуют и имеют правильные типы.
* `profile_id` всегда строка, уникальная внутри массива `profiles` .
* Минимум один профиль в `profiles` присутствует.

### Бизнес-логика

* Если `meta.active_profiles` пуст — выбирается первый профиль из `profiles`.
* Если `meta.active_profiles` задан — выборка выполняется строго по их порядку.
* При несоответствиях (отсутствие профиля, дубликаты) генерация отчётливой ошибки с привязкой к месту в конфиге.

---

# Семантические проверки по типам конфигов

> На текущем этапе в `config_loader.py` реализована Семантические проверочная логика.

## Внутреннний класс `_ConfigSemanticInvariantsByType`

### Роль в модуле

Класс отвечает за проверку **бизнес-правил и логических инвариантов**, которые невозможно или нежелательно описывать только средствами JSON Schema. Его задача — гарантировать, что загруженные конфиги не только структурно валидны, но и согласованы по смыслу. Проверки выполняются строго по активным профилям и останавливаются при первом нарушении (fail-fast).
...
* На вход подаются профили, прошедшие валидацию по JSON Schema и выбранные как активные.
* В случае успешной проверки метод `run` возвращает `None`.
* При нарушении правил формируется структурированная ошибка (`ConfigError`) с детальной диагностикой.
* Ожидается строгая детерминированность: одинаковые данные → одинаковый результат проверки.

### Допущения

* Проверки выполняются только над активными профилями (результат `_ConfigActiveProfileSelector`).
* Схемы v1 гарантируют корректные типы данных и базовую структуру.
* Если в профиле отсутствует ожидаемый ключ (например, `profile_id`), это трактуется как ошибка на более ранних этапах.
* Проверки ориентированы на fail-fast, поэтому отчёт формируется только по первой найденной ошибке.

### Бизнес-логика

* **auth**: проверяются обязательные поля для `form`, корректность настроек `csrf`, наличие токенов при `api_token.enabled=true`, наличие `params` в `success_checks`.

* **export**: проверяется ограничение `sheet_name_core_limit ≤ sheet_name_max_len ≤ 31`.

* **logging**: проверяется, что `ui.level` входит в список `events.levels`.

* **network**:
  
  * `rate_limit.burst_size ≥ requests_per_second`;
  * `retry_policy.timeout_budget_s` не меньше суммы задержек экспоненциального бэкоффа;
  * `session.request_timeout_s ≥ retry_policy.timeout_budget_s`.

* **site**:
  
  * непустой `page_container_selector`;
  * массивы `item_containers` и `fields` не пустые;
  * уникальность `container_id` и `field.name`;
  * при `value_selector.extract="attr"` поле `attr` обязательно и непустое.

* **pipeline, url, meta**: дополнительных семантических проверок нет.

---

# Интерполяция плейсхолдеров

На текущем этапе в `config_loader.py` **НЕ реализована** Интерполяция плейсхолдеров и класс `_ConfigPlaceholderFilling`.

## Out of scope класс `_ConfigPlaceholderFilling`

### Почему не реализован

Функционал интерполяции плейсхолдеров вида `${env:VAR}` и `${secret:KEY}` был признан нефункциональным для пользовательских конфигов по следующим причинам:

* Работа с секретами в пользовательских сценариях не имеет смысла, так как требует дополнительной настройки окружения.
* В текущем состоянии проекта отсутствуют сервисы, где использование секретов критично (например, подключения к БД или брокеру сообщений).
* Поддержка шага интерполяции в текущем виде добавила бы избыточные зависимости (`os.environ`, `streamlit.secrets`) и усложнила бы тестируемость.

### До какого момента реализация отложена

Реализация данного класса отложена до появления следующих условий:

* Введение понятия "системный конфиг" и разделение пользовательских и системных конфигураций.
* Появление сценариев, требующих безопасного управления секретами (БД, Kafka, внешние сервисы).
* Разработка отдельного модуля для работы с секретами и окружением, чтобы исключить прямые зависимости от внешних провайдеров и повысить модульность.

Таким образом, этап 2.6 «Интерполяция плейсхолдеров» зафиксирован в спецификации, но в текущей реализации пайплайна находится в состоянии *out of scope*. Реализация планируется в будущих итерациях.
